...
import { savePoint, saveRelPoint } from "../../redux/authReducer";
            ...
            >
              ...
              >
                <Text style={{ color: "#545454" }}>
                  第{item.lotteryResult.drawIssue + 1}期
                </Text>
                <Text style={{ color: "#545454" }}>
  ...
  useEffect(() => {
    ...
    betLogs.map((item, i) => {
      if (item.lotteryResult)
        if (tempLotteryResult != item.lotteryResult.result) {
          temp.push(arrTemp);
          arrTemp = [];
          arrTemp.push(item);
          tempLotteryResult = item.lotteryResult.result;
        } else {
          arrTemp.push(item);
        }
      return;
    ...
    result = temp.map((item, i) => {
      var tempResult = [];
      item.map((item, i) => {
        item.bettingResult.map((a, i) => {
          tempResult.push({
            result: a,
            ...
            <View style={styles.header_info_view}>
              ...
              <View style={styles.header_col}>
                ...
                <Text style={[styles.white_text]}>
                  回水:{" "}
                  {formatProfit(
                    !userInfo.relpoints ? 0 : userInfo.relpoints.backwater
                  )}
                </Text>
              </View>
              <View style={styles.header_col}>
                <Text style={[styles.white_text]}>
                  输赢:{" "}
                  {formatProfit(
                    !userInfo.relpoints ? 0 : userInfo.relpoints.relProfit
                  )}
                </Text>
                <Text style={[styles.white_text]}>
                  流水:{" "}
                  {formatProfit(
                    !userInfo.relpoints ? 0 : userInfo.relpoints.betAmount
                  )}
                </Text>
              </View>
            </View>
  ...
  useEffect(() => {
    ...
    socket.on(SOCKET_BET_JOIN_ROOM, (response) => {
      ...
      if (res.status == RES_STATUS_SUCCESS) {
        ...
        for (var i = 0; i < res.data.userInfo.relpoints.length; i++) {
          if (
            res.data.userInfo.relpoints[i].lotteryType ==
            currentLottery.lotteryType
          ) {
            relpoint = res.data.userInfo.relpoints[i];
          }
        }
        console.log(relpoint);
        dispatch(saveRelPoint({ relpoints: relpoint }));
        var temp = [];
    ...
    socket.on(SOCKET_BET_NEWTURN_EVENT1, (response) => {
      ...
      for (var i = 0; i < response.userInfos.length; i++)
        if (userInfo._id == response.userInfos[i]._id) {
          ...
          for (var j = 0; j < response.userInfos[i].relpoints.length; j++) {
            if (
              response.userInfos[i].relpoints[j].lotteryType ==
              currentLottery.lotteryType
            ) {
              console.log(response.userInfos[i].relpoints[j]);
              dispatch(
                saveRelPoint({
                  relpoints: response.userInfos[i].relpoints[j],
                })
              );
            }
          }
      ...
      if (response.betlogs != undefined && response.betlogs.length) {
        ...
        var content = (
          <View>
            ...
            <Text style={{ fontSize: 18 }}>
              {`${parseInt(response.drawIssue) + 1}`}期有效投注
            </Text>
            <Text>========================</Text>
