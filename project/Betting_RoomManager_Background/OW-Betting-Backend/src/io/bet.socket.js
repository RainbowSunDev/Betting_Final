...
const bet = (io, socket) => async ({
  ...
  var updatedPoints = MiddleBettingCalc(
    msg,
  ...
  // userone.beforePoint = userone.point; // save the before point.
  userone.point = updatedPoints.updateduserpoint; // save the updateuserpoint (minus calc)

  // update relpoints
  userone.relpoints.map((item, key) => {
    if (item.lotteryType == lotteryType) {
      // start backwater calculation part
      if (
        Math.floor(item.betAmount / 10000) + 1 ==
        Math.floor((item.betAmount + Number(updatedPoints.betAmount)) / 10000)
      ) {
        item.backwater += 130;
      }
      // end backwater calculation part

      item.betAmount += Number(updatedPoints.betAmount); //total betAmount
    }
  });
