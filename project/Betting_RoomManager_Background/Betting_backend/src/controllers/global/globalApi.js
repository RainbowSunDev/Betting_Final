    ...
    const request = https.request(each_api_url, (response) => {
      ...
      response.on("end", () => {
        ...
        arrGlobalApiLeftTimes[key] =
          ...
          new Date(body.result.data.serverTime).getTime();
        temp++;
        console.log("arrGlobalApiLeftTimes=========", arrGlobalApiLeftTimes);
  ...
  BetLogs.find({
    ...
    .then(async (betlogsforUpdate) => {
      if (betlogsforUpdate.length == 0) {
        console.log("NewTurnEvent ------- result is zero");
        // Emit NEWTURN Event
        Rooms.find().then((result) => {
          result.map((item, key1) => {
            // emit NEWTURN event
            console.log("New zero newturn event !!!!!!!!!!!!!");
            ...
            BetLogs.find({
              ...
              .then((betlogsdata) => {
                if (betlogsdata) {
                  console.log(
                    "leftTimeeeeeeeeeeeeeeeeeeeeeeeee1111",
        ...
        console.log("NewTurnEvent ------- result is not zero");
        // Emit NEWTURN Event
        Rooms.find().then((result) => {
          result.map(async (item, key1) => {
            // emit NEWTURN event
            console.log("New not zero turn real event !!!!!!!!!!!!!");

            console.log("data.preDrawIssue===>", data.preDrawIssue);
            ...
            BetLogs.find({
              ...
              .then((betlogsdata) => {
                if (betlogsdata) {
                  console.log("betlogsdata-----------------", betlogsdata);
                  console.log(
                    "leftTimeeeeeeeeeeeeeeeeeeeeeeeee",
                    new Date(data.drawTime).getTime() -
                      new Date().getTime() -
                      deltaTime
                  );
